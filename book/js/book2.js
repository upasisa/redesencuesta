...
document.addEventListener("DOMContentLoaded", function(evt) {
  ...
  for (let i=0, l=interactives.length; i<l; i++) {
    ...
    new_expand.onclick = new Function("", `openInteractive("${src}", ${w}, ${h});return 0;`);
  ...
  for (let i=0; i<pages.length; i++) {
    if ((i >= init_page_num) && (!pages[i].hasAttribute("num"))) {
      ...
      if (i < arabic_number_page) {
        tmp_num.textContent = toRoman((i - init_page_num) +419);
      }
      else {
        tmp_num.textContent = (i - init_page_num) +419;
      }
